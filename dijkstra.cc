...
void Dijkstra::RunForTarget(int seeked_source, int source, int target, int totalDistance) {
    if (debug) {
    ...
    for (int i = 0; i < outgoing_arcs.size(); i++) {
        ...
        if (arcTo != target)
            RunForTarget(seeked_source, arcTo, target, totalDistance);
    }
...
void Dijkstra::RunUntilAllTargetsAreReached(int source, const vector<int> &targets) {
    for (int i = 0; i < targets.size(); i++) {
        RunForTarget(source, source, targets[i], 0);
    }
    if (debug) {
